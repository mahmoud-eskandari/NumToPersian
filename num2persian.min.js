 ...
 **/
NumToPersian=function(){return spliter=" و ",zero="صفر",Letters=[["","یك","دو","سه","چهار","پنج","شش","هفت","هشت","نه"],["ده","یازده","دوازده","سیزده","چهارده","پانزده","شانزده","هفده","هجده","نوزده","بیست"],["","","بیست","سی","چهل","پنجاه","شصت","هفتاد","هشتاد","نود"],["","یكصد","دویست","سیصد","چهارصد","پانصد","ششصد","هفتصد","هشتصد","نهصد"],[""," هزار "," میلیون "," میلیارد "," بیلیون "," بیلیارد "," تریلیون "," تریلیارد "," کوآدریلیون "," کادریلیارد "," کوینتیلیون "," کوانتینیارد "," سکستیلیون "," سکستیلیارد "," سپتیلیون "," سپتیلیارد "," اکتیلیون "," اکتیلیارد "," نانیلیون "," نانیلیارد "," دسیلیون "," دسیلیارد "]],function(e){if(0===parseInt(e))return zero;if(e.length>66)return"خارج از محدوده";for(SplitedNumber=function(e){for("number"==typeof e&&(e=e.toString()),NumberLength=e.length%3,1===NumberLength?e="00"+e:2===NumberLength&&(e="0"+e),e=e.split(""),index=0,output=[],i=0;i<e.length;i++)i>0&&i%3==0&&index++,void 0!==output[index]?output[index]+=e[i]:output[index]=e[i];return output}(e),funcout=[],SplitLength=SplitedNumber.length,i=0;i<SplitLength;i++)SectionTitle=Letters[4][SplitLength-(i+1)],converted=(t=SplitedNumber[i],0===parseInt(t)?"":(parsedInt=parseInt(t),parsedInt<10?Letters[0][parsedInt]:parsedInt<=20?Letters[1][parsedInt-10]:parsedInt<100?(one=parsedInt%10,ten=(parsedInt-one)/10,one>0?Letters[2][ten]+spliter+Letters[0][one]:Letters[2][ten]):(one=parsedInt%10,hundreds=(parsedInt-parsedInt%100)/100,ten=(parsedInt-(100*hundreds+one))/10,out=[Letters[3][hundreds]],SecendPart=10*ten+one,SecendPart>0&&(SecendPart<10?out.push(Letters[0][SecendPart]):SecendPart<=20?out.push(Letters[1][SecendPart-10]):(out.push(Letters[2][ten]),one>0&&out.push(Letters[0][one]))),out.join(spliter)))),""!==converted&&funcout.push(converted+SectionTitle);var t;return funcout.join(spliter)}}(),String.prototype.toPersian=function(){return NumToPersian(this)},Number.prototype.toPersian=function(){return NumToPersian(parseInt(this).toString())};